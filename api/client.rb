module CGMiner; module API
  class Client
    ...
    def addpool(url, usr, pass)
      command(:addpool, url, usr, pass)
    end
    ...
    def removepool(index)
      command(:removepool, index)
    end
    ...
    def command(symbol, *parameters)
      ...
      results = telnet.cmd(
        { command: symbol.to_s, parameter: parameters.join(',') }.to_json
      )
